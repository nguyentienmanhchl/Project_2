...
io.on('connection', socket => {
    socket.on('joinRoom', ({ username, room }) => {
      const user = userJoin2(socket.id, username, room);
      socket.join(user.room);      
    });
    // Listen for chatMessage
    socket.on('chatMessage', msg => {
      const user = getCurrentUser2(socket.id);
...
// chat at groupchat
io.on('connection', socket => {
  socket.on('joinRoomChat', ({ username, room }) => {
